...
<body class="bg-background-light dark:bg-background-dark text-gray-800 dark:text-gray-100 min-h-screen font-display">
        ...
        <div class="flex-1 w-full max-w-5xl mx-auto p-4 md:p-8 lg:p-12">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    @if (session('success'))
                        <div
                            class="mb-4 p-4 bg-green-100 border-l-4 border-green-500 text-green-700 shadow-sm rounded-r-lg">
                            {{ session('success') }}
                        </div>
                    @endif
                    @if (session('error'))
            ...
            <div class="space-y-4">
                @forelse($bookings as $booking)
                    <div
                        ...
                        <div class="flex-1 flex flex-col justify-between gap-4">
                            <div>
                                <div class="flex items-center gap-3 mb-2">
                                    ...
                                    <span class="text-xs text-gray-500 font-medium tracking-wide uppercase">Ref:
                                        #BK-{{ $booking->id }}</span>
                                </div>
                                <h3
                                    class="text-2xl font-bold text-gray-900 dark:text-white flex flex-wrap items-center gap-2">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                <div class="flex items-center gap-3 text-gray-600 dark:text-gray-300">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span
                                            class="font-medium">{{ \Carbon\Carbon::parse($booking->trip->date_time)->format('M d, Y • h:i A') }}</span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3 text-gray-600 dark:text-gray-300">
                                    <div class="p-2 bg-primary/10 rounded-lg text-primary">
                        ...
                        <div
                            ...
                            <div>
                                <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Your Driver
                                </h4>
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-bold text-gray-900 dark:text-white">
                                            {{ optional($booking->trip->taxi->driver)->name ?? 'Unknown' }}</p>
                            ...
                            <div class="flex flex-col gap-2">
                                @php
                                    $departureTime = \Carbon\Carbon::parse($booking->trip->departure_datetime);
                                ...
                                @if ($booking->status !== 'cancelled')
                                    @if ($canCancel)
                                        <form action="{{ route('mybookings.cancel', $booking->id) }}" method="POST"
                                            ...
                                            @method('POST')
                                            <button type="submit"
                                                class="w-full bg-white dark:bg-transparent border border-red-200 text-red-500 hover:bg-red-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
                                    ...
                                    @else
                                        <div
                                            ...
                                            <span class="text-orange-600 text-xs font-medium">Non annulable
                                                (-24h)</span>
                                        </div>
                                    @endif
                                @else
                                    <div class="flex items-center gap-2 p-2 bg-gray-100 rounded-lg">
                                        ...
                                        <span class="text-gray-500 italic text-sm font-medium">Réservation
                                            annulée</span>
                                    </div>
                                @endif
                            </div>
                ...
                @empty
                    <div
                        class="text-center py-12 bg-surface-light dark:bg-surface-dark rounded-xl border border-dashed border-gray-300">
            ...
            <div
                ...
                <div class="relative z-10 max-w-lg">
                    ...
                    <button
                        class="bg-white text-primary hover:bg-gray-50 px-5 py-2 rounded-lg font-bold text-sm shadow transition-colors">Explore
                        Routes</button>
                </div>
                <div
